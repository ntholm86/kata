...
Together they produce **Autonomous Reasoning Fidelity (ARF)** — the external signal that the agent is genuinely reasoning about the situation and that the reasoning is visible enough for observers to judge.
...
A prescriptive instruction produces compliance, not understanding. The agent should arrive at specifics because they're the right answer for the situation, not because they were listed.

- **Ask, don't tell.** "What here doesn't earn its existence?" not "Look for unused imports."
- **State the goal, not the steps.** "Find where the system is asked to do too much" not "Functions over 50 lines."
- **Provide vocabulary, not answers.** Concepts as a thinking framework, not a lookup table.
- **Trust the interpretation.** If the agent's interpretation leads to a different answer than the checklist would have, the interpretation might be right and the checklist wrong.

**The test:** if you removed all specific examples and thresholds from a skill, would an intelligent agent still know what to do?
...
Autonomy without observability is not delegation — it is abdication. Every autonomous operation must produce a visible, continuous trail as a structural property of the operation itself.

```
Transparency → Trust → Autonomy
     ↓                    ↓
if lost, both decay; zero visibility means autonomy must be revoked
regardless of competence.
```

**The resolution requirement.** Evidence must exist at multiple resolutions: full (the complete reasoning exchange), indexed (key decisions/realizations highlighted within the full evidence), and digested (a 60-second summary). A single-resolution trail satisfies one observer at the cost of silently excluding the others.

**Fidelity marking.** Where the agent authors its own summary, mark it. "Reconstructed from agent memory" and "verbatim tool output" are different trust levels; conflating them is dishonest.

**The test:** if the human stepped away for an hour and came back, could they — at their resolution — reconstruct what the agent did, why, and whether to trust the results, from the trail alone?
...
Iterative loops declare convergence too early. A single evaluator can converge on its own blind spots. Score stability is necessary but not sufficient.

**Three simultaneous conditions:**

1. **Score agreement across distinct evaluator families.** Same-family evaluators (multiple versions of one model) count as one. A different family does not share the first family's blind spots.
2. **Zero material change.** Each evaluation ends with no changes to the artifact. If a run produces a diff, the convergence counter resets to zero — regardless of whether the score changed.
3. **Independent assessment, including of the measurement scheme.** A new model in the same conversation is *not* independent. A genuinely independent evaluator re-derives the measurement scheme before scoring; convergence on re-derivation validates the scheme, divergence is itself a finding.

**The minimum bar:** 3 consecutive runs, 3 distinct evaluator families, same score, zero artifact changes, at least one re-derivation of the measurement scheme that converged with the inherited scheme. Below this is improvement trajectory, not convergence.

**The corollary:** *if the loop is still producing changes, the system is still improving. Convergence is not a score — it is the absence of actionable findings across independent observers.*

## How they interact

Commander's Intent without Observable Autonomy is dangerous (you can't see how the agent is pursuing the goal). Observable Autonomy without Commander's Intent is theatre (you can see compliance, not interpretation). Convergence Is Silence completes the system — defining when work is done — and requires the other two: an evaluator that interprets independently, and a trail that is fully visible.

These principles do not *create* trust. They produce an **evidence substrate** on which trust *can* form — if observers actually consume the evidence, if the reasoning shown is correct (not just visible), and if the social and organizational conditions for delegation exist.

---

## For skill authors

Every skill in this suite must embody all three principles:

1. **Structure over prescription.** Define phases that shape work. Within each, ask questions that guide reasoning. Provide vocabulary, not answers.
2. **Continuous narration.** Every phase produces visible output. Reasoning must be the agent's own. "I found X because Y" not "Checklist item 3: checked."
3. **Trail recording is mandatory.** Every skill run appends to the target's trail. No run is invisible.
4. **Self-targeting must work.** If the skill can't be run on itself and produce meaningful results, it's too prescriptive or too vague.
# Principles of Autonomous Development
...
Together they produce **Autonomous Reasoning Fidelity (ARF)** — the external signal that the agent is genuinely reasoning about the situation and that the reasoning is visible enough for observers to judge. ARF is not a fourth principle; it is what the three principles measure when they hold simultaneously.
...
The operational standards (verification scripts, integrity snapshots, scoring rubric, metrics history) that support these principles live in [STANDARDS.md](./STANDARDS.md). They are tool prescriptions — not architectural constraints — and belong there, not here.
