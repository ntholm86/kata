...
| 6 | **Instruction Clarity** | CMMI REQM (L3) | Can any competent LLM follow the skill instructions without ambiguity? Are requirements (PRINCIPLES) unambiguous and complete? Are principles traceable to skills through clarity alone — without needing a formal matrix? | Clarity (kept, anchored; absorbs Traceability) |
...
| 8 | **Autonomous Reasoning Fidelity** | Auftragstaktik (Prussian mission-type command), Meaningful Human Control (autonomy ethics), Trust Calibration (Lee & See 2004), PRINCIPLES.md §1-2 | Two tests: **(1) Freedom of thought** — apply the Principle 1 test: remove all examples and thresholds from a skill; would a competent agent still know what to do? If yes, Commander's Intent holds — the skill defines the destination without prescribing the route. If no, the skill has drifted toward prescription and the agent is following a checklist, not reasoning. **(2) Trail integrity** — apply the Principle 2 test: can an absent human reconstruct what the agent did, why it did it, and whether to trust the results, from the GENBA trail alone, without asking? If yes, Observable Autonomy holds — the agent has earned the right to keep acting. If no, autonomy is unearned regardless of output quality. | *New — measures the suite's core differentiator: genuinely free reasoning made trustworthy through radical transparency* |
